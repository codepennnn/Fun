...
{
    string start = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");  // make sure start is formatted
    Tracelog(start, DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"),
             "HelpDesk_Complaint_Close", "No complaints to update");
    return;
}

public void Tracelog(string start, string end, string taskname, string status)
{
    string now = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");  // format UPDATEDAT too
    ...
    String s1 = "Insert into App_JUSCOTASKDETAILS (UPDATEDAT,UPDATEDBY,STARTTIME,ENDTIME,TASKNAME,STATUS) " +
                "values('" + now + "','000001','" + start + "','" + end + "','" + taskname + "','" + status + "')";
